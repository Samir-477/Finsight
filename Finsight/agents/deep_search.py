...
class DeepSearchAgent(Agent):
    ...
    def run(self, query: str) -> Dict[str, Any]:
        ...
        for iteration in range(self.max_iterations):
            ...
            guidance_prompt = (
                "You are assisting a financial analyst doing research on a company. "
                "Based on the snippets below, either:\n"
                "  A) Return exactly DONE if you have enough context, OR\n"
                "  B) Return a single refined search query string (no explanation, no quotes, "
                "just the raw query text, max 100 characters)\n\n"
                f"Current query: {current_query}\n"
                f"Snippets: {str(combined_snippets[:3])[:800]}"
            )
            ...
            # Extract just the query from the LLM response — previously the
            # entire response paragraph was used as the Serper search query.
            raw = guidance.strip()
            # Remove markdown fences
            raw = re.sub(r"```[\w]*\n?", "", raw).replace("```", "").strip()
            # If the response is DONE with extra text, stop
            if raw.upper().startswith("DONE"):
                break
            # Try to extract a quoted query first
            quoted = re.search(r'["\'](.*?)["\']', raw)
            if quoted and 10 <= len(quoted.group(1)) <= 120:
                current_query = quoted.group(1).strip()
            else:
                # Take only the first line, strip common LLM preamble phrases
                first_line = raw.split("\n")[0]
                first_line = re.sub(
                    r"^(try searching for|search for|refined query|query|next query)[:\-]?\s*",
                    "", first_line, flags=re.IGNORECASE
                ).strip().strip("\"'`")
                # Cap at 150 chars — Serper breaks on very long queries
                current_query = first_line[:150] if first_line else current_query
