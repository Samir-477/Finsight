...
class CodeExecutor:
    ...
    def run(self, code: str, initial_globals: Dict[str, Any] | None = None) -> ExecutionResult:
        # Provide standard builtins so LLM-generated code can use print(), len(),
        # range(), int(), float(), str(), dict(), list(), sum(), sorted(), etc.
        # The empty self.allowed_builtins dict previously caused NameError on every
        # builtin call, silently failing all perspective agent analysis chains.
        import builtins as _builtins
        globals_dict = {"__builtins__": vars(_builtins)}
        if initial_globals:
